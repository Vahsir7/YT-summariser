document.addEventListener('DOMContentLoaded', () => {
    ...
    getInsightsButton.addEventListener('click', async () => {
        ...
        if (!videoUrl) {
            insightsContentDiv.innerHTML = '<p>Please enter a YouTube video URL.</p>';
            return;
        }
        ...
        if (!match || !match[1]) {
            insightsContentDiv.innerHTML = '<p>Invalid YouTube video URL. Please enter a valid URL.</p>';
        ...
        try {
            ...
            insightsContentDiv.innerHTML = marked.parse(data.insights || 'No insights received.');
            ...
            errorDiv.style.display = 'block';
            errorDiv.textContent = `Error: ${error.message}. Please check your browser console for more details.`;
            videoContainer.innerHTML = '';
        } finally {
